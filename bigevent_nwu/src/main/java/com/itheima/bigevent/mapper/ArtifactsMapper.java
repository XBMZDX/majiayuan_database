...
public interface ArtifactsMapper {
    // 添加文物
    @Insert("INSERT INTO artifacts (serial_number, new_artifact_code, new_artifact_name, original_artifact_code, original_artifact_name, material1, completeness, artifact_description, quantity1, quantity2, dimensions, weight, excavation_relic, excavation_position, excavation_time, storage_method, storage_location, images, transfer_process, restoration_status, photographer, draftsperson, text_describer, notes, grading_status, testing_status, created_by, create_time, update_time) VALUES (#{serialNumber}, #{newArtifactCode}, #{newArtifactName}, #{originalArtifactCode}, #{originalArtifactName}, #{material1}, #{completeness}, #{artifactDescription}, #{quantity1}, #{quantity2}, #{dimensions}, #{weight}, #{excavationRelic}, #{excavationPosition}, #{excavationTime}, #{storageMethod}, #{storageLocation}, #{images}, #{transferProcess}, #{restorationStatus}, #{photographer}, #{draftsperson}, #{textDescriber}, #{notes}, #{gradingStatus}, #{testingStatus}, #{createdBy}, #{createTime}, #{updateTime})")
