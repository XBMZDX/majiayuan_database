...
const statusMap={draft:['草稿','info'],in_progress:['制作中','primary'],completed:['已完成','success'],reviewed:['已审核','warning'],returned:['退回修改','danger'],archived:['已归档','primary']}
    ...
    <aside class="result-list">
        ...
        <div class="cards">
            <article v-for="item in filtered" :key="item.id" :class="{active:item.id===activeId}" @click="$emit('select',item.id)">
                ...
                <div class="flags"><el-tag v-if="item.recommendedResult" size="small" type="primary">推荐成果</el-tag><el-tag v-if="item.selectedForArchive" size="small" type="success">正式档案</el-tag><el-button v-if="!['archived','reviewed'].includes(item.resultStatus)" link type="danger" :icon="Delete" @click.stop="$emit('delete',item)" /></div>
            </article>
