...
const emit = defineEmits(['update:activeTab', 'dirty', 'add-target', 'generate-task', 'start-task', 'select-task', 'create-record', 'save-record', 'submit-record', 'complete-task', 'handle-alert', 'create-project', 'upload-media', 'navigate'])
  ...
  <el-tabs v-model="tab" class="tabs">
    ...
    <el-tab-pane label="执行记录" name="execution">
      <div class="paper" v-if="record">
        ...
        <section><div class="section-head"><div><h4>同角度影像</h4><p>要求机位：{{ target.shootingPosition || '无固定机位' }}</p></div><el-upload v-if="record.monitoringStatus!=='submitted'" :auto-upload="false" :show-file-list="false" accept="image/*,.pdf" :on-change="file=>$emit('upload-media',file.raw)"><el-button :icon="Camera">上传本次影像</el-button></el-upload></div><el-alert v-if="target.requiresImage&&!record.media?.length" title="当前对象要求上传同角度影像" type="warning" :closable="false"/><div class="images"><figure><img v-if="target.baseline?.baselineFileUrl" :src="target.baseline.baselineFileUrl"><div v-else>无基准图</div><figcaption>修复后基准 · {{ target.baseline?.baselineDate }}</figcaption></figure><figure><img v-if="record.media?.[0]&&record.media[0].contentType?.startsWith('image/')" :src="record.media[0].fileUrl"><div v-else-if="record.media?.[0]">{{ record.media[0].fileName }}（非图片附件）</div><div v-else>等待上传本次影像</div><figcaption>本次监测 · {{ String(record.monitoringDate).slice(0,10) }}</figcaption></figure></div></section>
    ...
    <el-tab-pane label="预警处理" name="alerts">
      ...
      <article v-for="a in plan.alerts" :key="a.id" class="alert-row"><Warning/><div><small>{{ a.alertCode }} · {{ a.discoveredTime }}</small><h4>{{ a.alertTitle }}</h4><p>{{ a.alertDescription }}</p><span>触发：{{ a.thresholdDescription }}</span><div v-if="a.immediateAction" class="action-note">即时措施：{{ a.immediateAction }}<br>处理建议：{{ a.treatmentAdvice }}</div><div v-if="a.createdProjectId" class="project-note">已创建保护修复项目 #{{ a.createdProjectId }} · {{ a.projectCreatedTime }}</div></div><aside><el-tag :type="a.alertLevel==='critical'?'danger':'warning'">{{ a.alertLevel }}</el-tag><el-tag type="info">{{ a.alertStatus }}</el-tag><el-button size="small" type="warning" @click="$emit('handle-alert',a)">确认/处理</el-button><el-button size="small" :type="a.createdProjectId?'success':'primary'" @click="$emit('create-project',a)">{{ a.createdProjectId ? '进入新项目' : '创建修复项目' }}</el-button><el-button size="small" @click="$emit('navigate','disease',a.targetId)">病害调查</el-button></aside></article>
...
.project-note{margin-top:10px;padding:8px;border-left:3px solid #58a181;background:#fff;font-size:10px;line-height:1.7;color:#39715e}
