...
const handleImport = async () => {
    ...
    reader.onload = async (e) => {
        ...
        await artifactsBatchImportService(data.map(item => ({
            burialId: selectedBurialId.value, chariotIndex: selectedChariotIndex.value, // serialNumber 由后端自动分配
...
const downloadTemplate = () => { const h = ['序号','文物新编号','文物新名称','文物原始编号','文物原名称','材质1','材质2','完整度','视觉特征','数量1','数量2','尺寸','重量','出土遗迹','出土位置','出土时间','存放方式','文物流转过程','修复复原状况','拍照人','绘图人','文字描述人','备注','定级情况']; const ws = XLSX.utils.aoa_to_sheet([h]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'模板'); XLSX.writeFile(wb,'文物导入模板.xlsx') }
