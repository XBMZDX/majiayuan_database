...
const fetchBurialList = async () => {
    ...
    if (burialList.value.length > 0 && !selectedBurialId.value) { selectedBurialId.value = burialList.value[0].id; await loadBurialData() }
}
...
const confirmBatchDelete = () => {
    if (!selectedRows.value.length) { ElMessage.warning('请至少选择一件文物'); return }
    ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 件文物吗？`, '批量删除确认', { confirmButtonText: '确认删除', cancelButtonText: '取消', type: 'warning' })
        .then(async () => { await request.post('/artifacts/batch-delete', selectedRows.value.map(r => r.id)); ElMessage.success('删除成功'); cancelBatchMode(); loadBurialData() }).catch(() => {})
}
...
const handleImport = async () => {
    ...
    reader.onload = async (e) => {
        ...
        await artifactsBatchImportService(data.map(item => ({
            burialId: selectedBurialId.value, serialNumber: item['序号'],
            ...
            completeness: item['完整度']||'', quantity1: item['数量1']||null, quantity2: item['数量2']||null,
            dimensions: item['尺寸']||'', weight: item['重量']||'',
        ...
        ElMessage.success('导入成功'); importVisible.value = false; loadBurialData()
    }
    reader.readAsArrayBuffer(importFile.value)
}

const downloadTemplate = () => {
    const headers = ['序号','文物新编号','文物新名称','文物原始编号','文物原名称','材质1','材质2','完整度','视觉特征','数量1','数量2','尺寸','重量','出土遗迹','出土位置','出土时间','存放方式','文物流转过程','修复复原状况','拍照人','绘图人','文字描述人','备注','定级情况','科技检测情况']
    const ws = XLSX.utils.aoa_to_sheet([headers]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, '模板'); XLSX.writeFile(wb, '文物导入模板.xlsx')
}
...
const submitAdd = async () => { await request.post('/artifacts', { ...addForm.value, burialId: selectedBurialId.value }); ElMessage.success('添加成功'); addVisible.value = false; loadBurialData() }
...
<template>
    <div class="chariot-page">
        ...
        <el-card class="table-card" shadow="never">
            ...
            <el-table :data="pagedArtifacts" style="width:100%" @selection-change="handleSelectionChange">
                ...
                <template #empty><el-empty description="该墓葬暂无车出土文物" /></template>
            </el-table>
            <el-pagination v-model:current-page="pageNum" v-model:page-size="pageSize" :page-sizes="[10,20,50]" layout="total,sizes,prev,pager,next" :total="artifacts.length" background style="margin-top:16px;justify-content:flex-end" @current-change="onPageChange" />
        </el-card>
    </div>
    ...
    <el-dialog v-model="importVisible" title="导入文物" width="500px">
        <el-form label-width="100px"><el-form-item><el-button type="success" @click="downloadTemplate">获取模板</el-button></el-form-item><el-form-item label="Excel文件"><el-upload :auto-upload="false" :on-change="handleImportFile" accept=".xlsx,.xls" :limit="1"><el-button type="primary">选择文件</el-button></el-upload></el-form-item></el-form>
