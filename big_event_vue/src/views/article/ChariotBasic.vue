...
// ========== 车选择 ==========
const selectedChariotIndex = ref(1)  // 1=车1, 2=车2...
const chariotOptions = computed(() => Array.from({length: chariotCount.value}, (_, i) => ({label: '车' + (i+1), value: i+1})))
// ========== 流程树（左侧）—— 从 API 加载 ==========
const treeData = ref([])

const fetchTree = async () => { const bid = selectedBurialId.value; if (!bid) return; const res = await request.get('/admin/chariot-workflow/tree', {params:{burialId:bid, chariotIndex:selectedChariotIndex.value}}); treeData.value = (res.data || []).map(t => ({ id: t.id, label: t.label })) }
const saveTreeToDB = async () => { const bid = selectedBurialId.value; if (!bid) return; await request.put('/admin/chariot-workflow/tree', { burialId: bid, chariotIndex: selectedChariotIndex.value, nodes: treeData.value }) }
...
const saveDialog = () => {
    ...
    saveTreeToDB(); const bid2 = selectedBurialId.value; bid2 && request.put('/admin/chariot-workflow/timeline', { burialId: bid2, chariotIndex: selectedChariotIndex.value, items: timelineData.value })
}
...
const saveTimeline = () => {
    ...
    // 保存到数据库
    (selectedBurialId.value ? request.put('/admin/chariot-workflow/timeline', { burialId: selectedBurialId.value, chariotIndex: selectedChariotIndex.value, items: timelineData.value }) : Promise.resolve())
}

// ========== 时间轴（中间）—— 从 API 加载 ==========
const timelineData = ref([])
const fetchTimeline = async () => { const bid = selectedBurialId.value; if (!bid) { timelineData.value = []; return }; const res = await request.get('/admin/chariot-workflow/timeline', {params:{burialId:bid, chariotIndex:selectedChariotIndex.value}}); timelineData.value = res.data || [] }
...
const selectedChariotName = computed(() => '车' + selectedChariotIndex.value)

const onBurialChange = () => { selectedChariotIndex.value = 1; treeData.value = []; timelineData.value = []; fetchTree(); fetchTimeline() }
const onChariotChange = () => { fetchTree(); fetchTimeline() }
...
const submitAddContent = async () => {
    ...
    if (addType.value === 'note') {
        ...
        await request.post('/admin/chariot-workflow/note', { timelineId: selectedEvent.value.id, noteType: '工作记录', content })
    } else if (addType.value === 'image') {
        if (!uploadedImages.value.length) return ElMessage.warning('请先选择并上传图片')
        await Promise.all(uploadedImages.value.map(image => request.post('/admin/chariot-workflow/media', {
        ...
        await request.post('/admin/chariot-workflow/media', { timelineId: selectedEvent.value.id, mediaType, fileName: addForm2.value.fileName, fileUrl: addForm2.value.fileUrl })
    }
...
const loadArchiveData = async (timelineId) => {
    ...
    const [notesRes, mediaRes] = await Promise.all([
        request.get('/admin/chariot-workflow/note/' + timelineId),
        request.get('/admin/chariot-workflow/media/' + timelineId)
    ])
...
const addNote = async () => {
    ...
    await request.post('/admin/chariot-workflow/note', { timelineId: selectedEvent.value.id, ...newNote.value })
...
const deleteNote = async (id) => { await request.delete('/admin/chariot-workflow/note/' + id); loadArchiveData(selectedEvent.value?.id) }
const deleteMedia = async (id) => { await request.delete('/admin/chariot-workflow/media/' + id); loadArchiveData(selectedEvent.value?.id) }
...
<template>
    <div class="workflow-page">
        <!-- 墓葬 + 车选择器 -->
        <div class="burial-selector" style="display:flex;gap:12px;align-items:center">
            ...
            <el-select v-if="chariotCount >= 2" v-model="selectedChariotIndex" placeholder="请选择车" style="width:100px" size="large" @change="onChariotChange">
                <el-option v-for="c in chariotOptions" :key="c.value" :label="c.label" :value="c.value" />
            </el-select>
            <span v-if="chariotCount === 1" style="font-size:14px;color:#1D2129;white-space:nowrap">车1</span>
        </div>
                ...
                <div v-if="detailData" class="detail-panel">
                    ...
                    <div class="detail-section">
                        <el-descriptions :column="1" border size="small">
                            ...
                            <el-descriptions-item label="所属车">{{ selectedChariotName }}</el-descriptions-item>
        ...
        <div v-if="detailData" class="archive-dialog">
            <div class="archive-summary-cover">
                <div>
                    ...
                    <p>{{ treeData.find(t=>t.id===detailData.flowId)?.label || '-' }} · {{ selectedChariotName }} · {{ detailData.date }}</p>
                </div>
