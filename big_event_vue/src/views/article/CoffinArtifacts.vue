...
// 棺选择：1=棺1, 2=棺2...
const selectedCoffinIndex = ref(1)
// 棺列表（1..coffinCount）
...
const calcStats = (list) => {
    ...
    return { total, materials: Object.entries(matMap).map(([name, count]) => ({ name, count })), completeness: Object.entries(compMap).map(([name, count]) => ({ name, count })) }
}
...
const loadArtifacts = async () => {
    ...
    artifacts.value = (artRes.data || []).filter(a => a.coffinIndex === selectedCoffinIndex.value)
...
<template>
    <div class="coffin-page">
        <div class="toolbar">
            <div style="display:flex;gap:12px;align-items:center">
                ...
                <el-select v-if="coffinCount >= 2" v-model="selectedCoffinIndex" placeholder="请选择棺" @change="onCoffinChange" style="width:100px" size="large">
                    <el-option v-for="c in coffinOptions" :key="c.value" :label="c.label" :value="c.value" />
                </el-select>
                <span v-if="coffinCount === 1" style="font-size:14px;color:#1D2129">棺1</span>
            </div>
        ...
        <div class="stats-row">
            <div class="stat-card"><div class="stat-value">{{ stats.total }}</div><div class="stat-label">棺出土文物总数</div></div>
        ...
        <el-card class="table-card" shadow="never">
            <template #header><span style="font-weight:600">棺出土文物列表 ({{ selectedBurial?.burialNo || '' }} - 棺{{ selectedCoffinIndex }})</span></template>
            ...
            <el-table :data="pagedArtifacts" style="width:100%" @selection-change="handleSelectionChange">
                ...
                <template #empty><el-empty description="该棺暂无出土文物" /></template>
            </el-table>
    ...
    <ArtifactDetailDrawer v-model="detailVisible" :artifact="detailData" @saved="loadArtifacts" />
