...
const fetchTimeline = async () => { if (!selectedCoffinId.value) { timelineData.value = []; return }; const res = await request.get('/admin/coffin-workflow/timeline', {params:{coffinId:selectedCoffinId.value}}); timelineData.value = res.data || [] }
...
const submitAddContent = async () => {
    ...
    if (addType.value === 'note') {
        ...
        await request.post('/admin/coffin-workflow/note', { timelineId: selectedEvent.value.id, noteType: '工作记录', content })
    } else if (addType.value === 'image') {
        if (!uploadedImages.value.length) return ElMessage.warning('请先选择并上传图片')
        await Promise.all(uploadedImages.value.map(image => request.post('/admin/coffin-workflow/media', {
        ...
        await request.post('/admin/coffin-workflow/media', { timelineId: selectedEvent.value.id, mediaType, fileName: addForm2.value.fileName, fileUrl: addForm2.value.fileUrl })
    }
...
const loadArchiveData = async (timelineId) => {
    ...
    const [notesRes, mediaRes] = await Promise.all([
        request.get('/admin/coffin-workflow/note/' + timelineId),
        request.get('/admin/coffin-workflow/media/' + timelineId)
    ])
...
const addNote = async () => {
    ...
    await request.post('/admin/coffin-workflow/note', { timelineId: selectedEvent.value.id, ...newNote.value })
...
const deleteNote = async (id) => { await request.delete('/admin/coffin-workflow/note/' + id); loadArchiveData(selectedEvent.value?.id) }
const deleteMedia = async (id) => { await request.delete('/admin/coffin-workflow/media/' + id); loadArchiveData(selectedEvent.value?.id) }
                ...
                <div v-if="detailData" class="detail-panel">
                    ...
                    <div class="detail-section">
                        <el-descriptions :column="1" border size="small">
                            ...
                            <el-descriptions-item label="所属棺">{{ selectedCoffinName }}</el-descriptions-item>
        ...
        <div v-if="detailData" class="archive-dialog">
            <div class="archive-summary-cover">
                <div>
                    ...
                    <p>{{ treeData.find(t=>t.id===detailData.flowId)?.label || '-' }} · {{ selectedCoffinName }} · {{ detailData.date }}</p>
                </div>
